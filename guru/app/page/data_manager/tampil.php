...
<?php  if ($_COOKIE['hak_akses'] == "admin") { ?>
    ...
    <a target="blank" href="cetak.php?berdasarkan=data_manager&jenis=xls&pakaiperperiode=<?php echo $status_pakaiperperiode; ?>">
        <?php btn_export('Export Excel'); ?>
    </a>

    <a target="blank" href="cetak.php?berdasarkan=data_manager&jenis=print&pakaiperperiode=<?php echo $status_pakaiperperiode; ?>">
        <?php btn_cetak('Cetak'); ?>
    </a>
...
<?php } ?>
            ...
            <tbody>
                ...
                if (isset($_GET['Berdasarkan']) && !empty($_GET['Berdasarkan']) && isset($_GET['isi']) && !empty($_GET['isi'])) {
                    ...
                    $querytabel = "SELECT * FROM data_manager where $berdasarkan like '%$isi%'  LIMIT $startRow ,$dataPerPage";
                    $querypagination = "SELECT COUNT(*) AS total FROM data_manager where $berdasarkan like '%$isi%'";
                } else {
                    $querytabel = "SELECT * FROM data_manager  LIMIT $startRow ,$dataPerPage";
                    $querypagination = "SELECT COUNT(*) AS total FROM data_manager";
                }
                    ...
                    <tr class="event2">	
                        ...
                        <td class="th_border cell" align="center" width="200">
                            <table border="0">
                                <tr>
                                    <td>
                                        <a href="<?php index(); ?>?input=detail&proses=<?= encrypt($data['id_manager']); ?>">
                                    ...
                                    <?php  if ($_COOKIE['hak_akses'] == "admin") { ?>
                                    <td>
                                        <a href="<?php index(); ?>?input=edit&proses=<?= encrypt($data['id_manager']); ?>">
                                        <?php btn_edit('Edit'); ?>
                                        </a>
                                    </td>
                                    <td>
                                        <a href="<?php index(); ?>?input=hapus&proses=<?= encrypt($data['id_manager']); ?>">
                                        <?php btn_hapus('Hapus'); ?>
                                        </a>
                                    </td>
                        ...
                        <!--h <td align="center"><?php echo $data['id_manager']; ?></td> h-->
